...
public class DownloadInvoiceAfterPurchaseOrder extends TestBase{
	...
	@Test
	public void userCanVerifyAddressDetailsInCheckoutPage() throws InterruptedException {
		...
		assertTrue(mypage.confirmationmessage.getText().contains("ACCOUNT CREATED!"));
		mypage.redirectToTheNextPage();
		assertTrue(myaccountpage.message.getText().contains("Logged in as "+name));
		myaccountpage.userCanNavigateToProductsPage();
		...
		assertEquals(checkkoutpage.fullname1.getText(),"Mr. "+firstname+" "+lastname);
		System.out.println(checkkoutpage.fullname1.getText());
		assertEquals(checkkoutpage.address1.getText(),address);
		System.out.println(checkkoutpage.address1.getText());
		assertEquals(checkkoutpage.fullname2.getText(),"Mr. "+firstname+" "+lastname);
		System.out.println(checkkoutpage.fullname2.getText());
		assertEquals(checkkoutpage.address2.getText(),address);
		System.out.println(checkkoutpage.address2.getText());
		...
		assertTrue(paymentpage.confirmatiommessage.getText().contains("Congratulations! Your order has been confirmed!"));
		System.out.println(paymentpage.confirmatiommessage.getText());
		paymentpage.userCanDownloadInvoice();
		paymentpage.userCandeleteHisAccount();
		assertEquals(paymentpage.deletemessage.getText(),"ACCOUNT DELETED!");
	}
