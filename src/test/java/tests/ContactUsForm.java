...
public class ContactUsForm extends TestBase{
	...
	String subject=faker.lorem().characters(20);
	@Test
	public void UserCanFillContactForm() throws InterruptedException, AWTException {
		...
		mypage.userCanLogout();
		page.userOpenContactUsPage();
		contactpage=new ContactUsPage(driver);
		//assertTrue(driver.getCurrentUrl().contains("contactc_us"));
		//assertTrue(contactpage.welcomemessage.getText().contains("Get In Touch"));
		System.out.println(driver.getCurrentUrl());
		System.out.println(contactpage.welcomemessage.getText());
		contactpage.userFillContactForm(name, email,subject,message,filename);
		Alert alert=driver.switchTo().alert();
		alert.accept();
		assertEquals(contactpage.successmessage.getText(),"Success! Your details have been submitted successfully.");
		System.out.println(contactpage.successmessage.getText());
		contactpage.userNavigateToHomePage();
